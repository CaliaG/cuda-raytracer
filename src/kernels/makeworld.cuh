...
__device__ void make_box(Hittable **s, int &i,
  ...
  i++;
  s[i] = new XYRect(p0.x(), p1.x(), p0.y(), p1.y(), p1.z(), mptr);
  i++;
  s[i] = new XYRect(p0.x(), p1.x(), p0.y(), p1.y(), p0.z(), mptr);

  i++;
  s[i] = new XZRect(p0.x(), p1.x(), p0.z(), p1.z(), p1.y(), mptr);
  i++;
  s[i] = new XZRect(p0.x(), p1.x(), p0.z(), p1.z(), p0.y(), mptr);

  i++;
  s[i] = new YZRect(p0.y(), p1.y(), p0.z(), p1.z(), p1.x(), mptr);
  i++;
  s[i] = new YZRect(p0.y(), p1.y(), p0.z(), p1.z(), p0.x(), mptr);
}

__global__ void
make_empty_cornell_box(Hittables **world, Hittable **ss, curandState *randState) {
  // declare objects
  if (threadIdx.x == 0 && blockIdx.x == 0) {
